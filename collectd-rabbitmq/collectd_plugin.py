...
class CollectdPlugin(object):
    ...
    node_stats = ['disk_free', 'disk_free_limit', 'fd_total',
                  ...
                  'proc_total', 'proc_used', 'processors',
                  'run_queue', 'sockets_total', 'sockets_used',
                  'gc_num', 'gc_bytes_reclaimed', 'context_switches',
                  'io_write_bytes', 'io_read_count', 'io_read_bytes',
                  'io_read_avg_time', 'io_write_count', 'io_write_avg_time',
                  'io_sync_count', 'io_sync_avg_time', 'io_seek_count',
                  'io_seek_avg_time', 'io_reopen_count', 'mnesia_ram_tx_count',
                  'mnesia_disk_tx_count', 'msg_store_read_count',
                  'msg_store_write_count', 'queue_index_journal_write_count',
                  'queue_index_write_count', 'queue_index_read_count',
                  'io_file_handle_open_attempt_count',
                  'io_file_handle_open_attempt_avg_time', 'connection_created',
                  'connection_closed', 'channel_created', 'channel_closed',
    ...
    memory_stats = {'default': ["connection_readers", "connection_writers",
                                "connection_channels",  "connection_other",
                                "queue_procs", "queue_slave_procs", "plugins",
                                "other_proc", "metrics", "mgmt_db", "mnesia",
                                "other_ets", "binary", "msg_index", "code",
                                "atom", "other_system", "allocated_unused",
	...
	if stats is None:
            return None
	for subtree_name, keys in self.memory_stats.items():
		for stat_name in keys:
			if subtree_name == "default":
				value = stats.get('memory', {}).get(stat_name, 0)
			else:
