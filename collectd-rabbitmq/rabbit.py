...
class RabbitMQStats(object):
    ...
    def get_nodes(self):
        ...
        """
	global NODEID
        #return self.get_info("nodes/rabbit@"+ NODEID +"?memory=true") or list()
        return self.get_info("nodes") or list()
    nodes = property(get_nodes)

    # Memory stats
    def get_nodes_memory_stats(self,node_id):
        """
        Return a raw of memory stats data.
        """
	collectd.debug("Getting memory stats %s" % node_id)
        return self.get_info("nodes/rabbit@" + node_id + "/memory") or list()
